...
3. 依存追加フロー（チートシート）1. どこで使う？
   　- Web だけ → apps/web
   　- 複数で共有 → packages/\* に切り出し
   　- 開発補助 → ルート 2. インストールコマンド
...
4. 環境変数のルール（Next 15 / Supabase 前提）
   • クライアントで参照する値は NEXT*PUBLIC* を付けて apps/web の .env.local に置く。
...
with:
node-version: '20'
cache: 'npm' - run: npm ci - run: npm run lint - run: npm run typecheck - run: npm run test - run: npm run build
...
8. 即時導入チェックリスト
   ...
   • .env.local は apps/web 管理（NEXT*PUBLIC*\* と機密の切り分け） ￼
