...
- `git commit --no-verify` や `git push --no-verify` の使用を絶対に禁止する。
  - 理由: 品質フック（lint/typecheck/test 等）をバイパスし、品質低下・不具合混入のリスクが高まるため。
  - 代替: フックで失敗した場合は根本原因を特定し修正し、再実行する。

- ESLint抑制コメントの使用を絶対に禁止する。
  - 対象: `// eslint-disable`, `// eslint-disable-next-line`, `// eslint-disable-line` などすべての形式
  - 併せて TypeScript 抑制コメント（`// @ts-ignore`, `// @ts-nocheck`, `// @ts-expect-error`）も原則禁止とする
  - 理由: ルール逸脱の常態化により技術的負債/不具合温床となるため。
  - 代替: コード/型/変換層の是正（例: 厳密な型付け、DTO/Mapperでの明示プロパティ抽出、クエリ最小化、テスト補強）。
  - 例外: なし（緊急時も禁止）。どうしても必要なケースは事前にPM承認のうえ、専用ブランチ・短期除去計画（期日）・PRテンプレに記録し、24h以内に解消する。
...
2. **ローカル品質チェック必須**: コミット・プッシュ前に必ず実行
   ...
   - これによりリモートCI失敗を防ぐ
   - **絶対に守ること**:
     - ローカルでパスしない限りコミット・プッシュ禁止
     - `--no-verify` の利用禁止（フックを回避しない）
