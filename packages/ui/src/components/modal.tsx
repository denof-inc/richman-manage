...
const Modal: React.FC<ModalProps> = ({ isOpen, onOpenChange, children }) => {
...
const ModalTrigger: React.FC<ModalTriggerProps> = ({ asChild, children }) => {
  return <>{children}</>;
};
...
const ModalContent: React.FC<ModalContentProps> = ({ className, children, ...props }) => {
  return (
    <div
      className={cn(
        'fixed left-1/2 top-1/2 z-50 grid w-full max-w-lg -translate-x-1/2 -translate-y-1/2 gap-4 border border-gray-200 bg-white p-6 shadow-lg sm:rounded-lg',
        className
      )}
      {...props}
    >
...
const ModalDescription: React.FC<ModalDescriptionProps> = ({ className, children, ...props }) => {
  return (
